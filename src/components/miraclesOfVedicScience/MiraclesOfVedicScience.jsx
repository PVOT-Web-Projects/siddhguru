import wisdomImage from "../../images/coreWisdomImage.png";
import "./miracleOfVedicScience.scss";
import { motion } from "framer-motion";

const MiraclesOfVedicScience = () => {
  return (
    <div className="miraclesOfVedicScience">
      <div className="miraclesOfVedicScience_wrapper">
        <motion.div
          className="miraclesOfVedicScience_wrapper_image"
          initial={{
            opacity: 0,
            // if odd index card,slide from right instead of left
          }}
          whileInView={{
            opacity: 1,
            transition: {
              duration: 1, // Animation duration
            },
          }}
          viewport={{ once: true }}
        >
          <img src={wisdomImage} alt="wisdomImage" />
          <div className="imageHeading">{"Miracles of Vedic Science"}</div>
          <div className="miraclesOfVedicScienceText">
            <div className="left">
              <p>
                {
                  "During his sojourns in the Himalayas, Siddhguru Sri Siddheshwar Brahmrishi Gurudev undertook onerous Tapasya to unravel the secrets of Vedic Science. He developed a keen interest in Vedic Science and attained mastery over that sacred knowledge. A pupil of masters such as Maha Tapasvi Siddhiraj Pragyachakshuji and Brahmrishi Devraha Baba, Sri Siddhguru learned Vedic Science to heal and help people seeking his support."
                }
              </p>
              <p>
                {
                  " The ancient wisdom of Vedic Science offers us a profound understanding of the interconnectedness between an individual, the cosmos, and the Parmatma. It suggests a holistic approach to life that delves into the metaphysical nature of a living being."
                }
              </p>
            </div>
            <div className="right">
              <p>
                {
                  "Embracing Vedic wisdom in our everyday life is the only way to sail through the Bhavsagar."
                }
                <br /> {"- Sri Siddhguru"}
              </p>

              <p>
                {
                  "With small amendments to what we think and how we live, we can take our consciousness to higher levels. That’s when we can truly experience the miracles of Vedic Science. Sri Siddhguru believes this knowledge should reach everyone on the face of the earth, saving them from unnecessary suffering."
                }
              </p>
              <p>
                {
                  "With Sri Siddhguru as your guiding light, you can embrace a new, more powerful 'Vedic' perspective in your life."
                }
              </p>
            </div>
          </div>
        </motion.div>
        <div className="miraclesOfVedicScienceText_mobile">
          <div className="left">
            <p>
              {
                "During his sojourns in the Himalayas, Siddhguru Sri Siddheshwar Brahmrishi Gurudev undertook onerous Tapasya to unravel the secrets of Vedic Science. He developed a keen interest in Vedic Science and attained mastery over that sacred knowledge. A pupil of masters such as Maha Tapasvi Siddhiraj Pragyachakshuji and Brahmrishi Devraha Baba, Sri Siddhguru learned Vedic Science to heal and help people seeking his support."
              }
            </p>
            <p>
              {
                " The ancient wisdom of Vedic Science offers us a profound understanding of the interconnectedness between an individual, the cosmos, and the Parmatma. It suggests a holistic approach to life that delves into the metaphysical nature of a living being."
              }
            </p>
          </div>
          <div className="right">
            <p>
              {
                "Embracing Vedic wisdom in our everyday life is the only way to sail through the Bhavsagar."
              }
              <br /> {"- Sri Siddhguru"}
            </p>

            <p>
              {
                "With small amendments to what we think and how we live, we can take our consciousness to higher levels. That’s when we can truly experience the miracles of Vedic Science. Sri Siddhguru believes this knowledge should reach everyone on the face of the earth, saving them from unnecessary suffering."
              }
            </p>
            <p>
              {
                "With Sri Siddhguru as your guiding light, you can embrace a new, more powerful 'Vedic' perspective in your life."
              }
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};
export default MiraclesOfVedicScience;
